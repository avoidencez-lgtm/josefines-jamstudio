...
1. `src/components/RoomTools.tsx` has one `ROOM_TOOLS` registry keyed by the existing `ScreenId`. Each entry supplies a title, description and component. `App` mounts the tools once; inactive rooms are hidden so scratch drafts survive navigation. Tools perform no native work on mount. Subscribe only to needed store fields, not high-frequency telemetry.
