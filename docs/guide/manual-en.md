...
The top transport plays, pauses, stops, records, sets count-in and toggles loops. Stage shows the current/next chord and live position. The tuner starts off so tempo and bar stay visible. Transport tempo clamps 20–300 BPM; charts, Write and the rehearsal setlist stay 40–240. These are the chart and band settings. That panel selects a chart and matching style, changes intensity and band/click volume, and enables the tuner or a reference tone. Source stays on the jam band or requests Lyria RealTime. Jo can call lyria_vibe to change prompts and generate_track for a Lyria 3 or ElevenLabs file; both stay not configured without a key. With a Google Gemini key and JAM_LIVE=1, Lyria opens the provider WebSocket in Rust and plays its bounded audio stream; the WebView never receives the key or plays that stream. If that socket closes or fails during streaming, Lyria reconnects once with the latest settings and a fresh keychain read. Prompt or protocol errors stop without retrying. Without either prerequisite it stays not configured. Starting the band or a song stops Lyria, and starting Lyria stops the band. Lyria BPM is a request, not the band clock. Cues request Fill, Crash, Stop or Ending. Some changes are queued for the next bar and shown as pending. Part mutes affect drums, bass or comp; follow-energy follows measured guitar level, not musical intent. Shapes shows a playable fingering for the chord now and, smaller, the chord next, root in amber; the numbered buttons pick another shape. These are theory suggestions in standard tuning, not a transcription of what was played.
...
Invalid recorded tempo, time signature, section timing or MIDI notes stop the export before any bundle files are written. Guitar-layer metadata must be readable and contain at most 16 layers; otherwise the export stops with an error. Recover the take metadata from a backup before trying again; the app never fixes musical timing by silently clamping it. Original WAV recordings are kept. Each export uses a new folder; repeated exports add -2, -3 and so on to the take folder name. Previous exports and DAW projects saved there remain untouched. A failed export removes its own partial folder, or reports its location if cleanup fails. New reference recordings with a confirmed beat map export recorded tempo changes, ramp speeds and repeated sections. REAPER enables Guitar DI and Band, and mutes Master and silent generated parts. WAV timing stays intact. Lead-ins and partial starts can give different DAW bar numbers. Pauses and areas outside the confirmed grid are labelled; outside it, the nearest beat interval supplies an extrapolated tempo. Older takes without recorded timing retain constant tempo. Invalid or incomplete timing stops export.
...
In Write → Record & layers → Hands-free controls, choose MIDI input. Click Learn beside an action, press the pedal once, then enable pedal actions. Learning saves without executing the action. Actions include Keep this riff, Record or save this take, Play or stop this, Loop this selected section, Loop this next section, Keep this version, Cue the next setlist entry and Exit this loop. Capture must be armed before Keep. Bindings persist; the live connection and enable switch are deliberate each session.
...
Settings → First run lists the one-time setup: audio devices, loopback, keychain keys, a virtual MIDI port, sample packs, Songs residual and Load this minus-guitar mix, Sessions Progress from take files and README.txt export, then signing. Opening Logic and real-song residual at or below -6 dB stay not claimed. Re-open it any time. The written guitarist guide is docs/guide/setup.md (cabling, unsigned Mac note, troubleshooting). Sample pack assets-v1 is published with recorded SHA-256 values for standard-rock-kit-acoustic.zip and freepats-bass-comp.zip (CC0 acoustic drums, FreePats bass and FM piano). Download needs JAM_LIVE=1. After unpack, the band plays the acoustic kit.json/WAVs and bass.sf2/comp.sf2 from JosefinesJamstudio/assets. A missing pack stays on the synthetic kit or sine voices and says so. Test this key stays not configured; Check this key status looks only in the OS keychain. These are the diagnostics. It reports canvas meter and playhead rAF fps after Stage paints. Sample this idle CPU reads this process; DESIGN idle CPU under 3 % is not proven without a desktop WebView+engine idle fixture. Signing and notarisation are not configured. Installers stay unsigned until an Apple Developer account and Windows signing exist. On Mac use right-click Open or xattr -dr com.apple.quarantine. This is not a signed release claim.
...
Speech usage shows submitted STT seconds and TTS characters, including failed or interrupted requests, plus an estimate and the number of unpriced calls for the latest 10,000 valid log entries across all providers. Older entries remain in the log. If the usage log cannot be read, Settings shows the error instead of treating usage as zero. If numeric usage values are negative or their totals are too large, Settings hides that provider's amounts and asks you to check the local usage log. Other providers still show their totals. Saved entries are kept. In Jo AI → Open the voice setup, enter your Scribe v2 is priced in USD per hour and Flash v2.5 is priced in USD per 1,000 characters, then Save this voice setup. Blank means unknown; zero is an explicit zero estimate. No rates are assumed. Each request keeps its original estimate when prices change. Allowances, taxes, voice-specific charges and the final provider invoice are not calculated. Older log entries remain readable but cannot gain missing speech units.
