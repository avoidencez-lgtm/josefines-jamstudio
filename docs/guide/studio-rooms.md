...
Every room has a distinct icon and a permanent label. On wider windows the sidebar
also explains the task; on compact windows the labels remain. The transport and
**Assistant** button stay at the top. Opening the assistant never changes rooms.
...
Library drafts and Jo conversations survive moving between rooms during this app
session. Save a chart before opening another, or deliberately discard its changes.
Write and Film keep their existing explicit save/version workflows. Unsaved work
is not guaranteed across restarts. The browser preview does not write files.
...
Songs supports local import/reference playback, not automatic stems, chord
recognition or time-stretch. FFmpeg/ffprobe are required for media import/render;
no installer or model weights are bundled. MIDI controls say when disconnected.
No browser audio is produced. Real devices and model accounts require the desktop
app and their one-time setup.
