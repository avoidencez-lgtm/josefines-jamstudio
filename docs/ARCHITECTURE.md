...
silences frames whose *source* generation no longer matches the loaded song.
Speed/key changes keep the same source generation so leftover processed audio
still plays; their parameter serial stays in the stamp for the chord readout. Underruns hold the
