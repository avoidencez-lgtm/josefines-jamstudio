...
- `deny.toml`: licence allowlist Apache-2.0, MIT, BSD-2-Clause, BSD-3-Clause, ISC, 0BSD, Zlib, Unicode-3.0, CC0-1.0, MPL-2.0 only if a crate has no alternative (record the exception with a comment); deny GPL, LGPL, AGPL. `scripts/check-js-licences.mjs` reads `pnpm licenses list --json` and applies the same allowlist.
...
- Env: `JAM_HEADLESS=1` selects `NullOutput`; `JAM_FAKE_INPUT=<wav>` selects `FileInput` for the guitar input. **As built:** those WAVs are gitignored and `scripts/gen-fixtures.mjs` was never added; a missing fake-input path falls back to a 440 Hz sine. **Target:** committed `tests/fixtures/audio/sine-440.wav` and `guitar-e-blues-120.wav` with a README.
...
- **Target** names `src-tauri/src/net/elevenlabs.rs`, `net/gemini.rs` and `net/registry.rs`. **As built:** ElevenLabs is `src-tauri/src/net/voice.rs`; Gemini is a `PROVIDERS` row in `src-tauri/src/net.rs`; the allow-list is `ProviderEntry { id, base_url, auth, description }`.
