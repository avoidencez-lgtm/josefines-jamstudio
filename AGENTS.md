...
Tauri 2.11.x; Rust stable (MSRV 1.88 pinned as `rust-version`, edition 2021) with `cpal`, `rtrb`, `hound`, `pitch-estimate`, `midir`, `keyring`, `rusqlite`, `tokio`, `reqwest` (rustls), `serde`; React 19, Vite, TypeScript, Tailwind v4, zustand, `tonal`, zod, `@phosphor-icons/react`, Biome, vitest; corepack pnpm. Text providers are reached through the registry in `src/lib/jo/providers.ts` and the Rust `provider_fetch` proxy. Offline reference practice copies use vendored Signalsmith Stretch/Linear through `cxx` and bundled Symphonia/Rubato for audio decoding and rate conversion (FFmpeg remains a separate Film tool). Native references play through the existing output/recording queue with seconds loops and per-stem mixing. ElevenLabs separation and local stem ZIP import use the media provider registry and bounded native extraction. Local low-confidence tempo/chord/key analysis reuses RealFFT. Live per-stem speed/key processing reuses the vendored DSP on the render worker. Real provider/guitar-removal acceptance, analysed-grid playback and realtime provider streams remain roadmap work. No Python, no Node sidecar, no plugin framework, no dynamic loading in v1.
